...
const TX_STATUS_QUERY = `query AletheiaTransactionStatus($transactionId: HexEncoded!) { transactions(offset: {identifier: $transactionId}) { __typename ... on RegularTransaction { identifiers block { height hash } transactionResult { status } } } }`;
async function verifyIndexedTransaction(row, env) {
  const indexer = env.MIDNIGHT_INDEXER_URL || 'https://indexer.preprod.midnight.network/api/v3/graphql';
  try {
    ...
    if (transaction.transactionResult?.status === 'SUCCESS') return { state: 'confirmed', blockReference: String(transaction.block?.hash || transaction.block?.height || row.block_reference || '') };
...
async function handleClaimPatch(request, env) {
  const body = await request.json().catch(() => null); if (!body || !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(body.id || '') || !/^[a-f0-9]{64}$/i.test(body.txHash || '') || body.network !== 'preprod' || typeof body.contractAddress !== 'string' || body.contractAddress.length > 200 || !body.contractAddress) return json({ error: 'Complete Compact Preprod transaction evidence is required.' }, 400);
