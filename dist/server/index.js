...
async function handleClaimPatch(request, env) {
  const body = await request.json().catch(() => null); if (!body || typeof body.id !== 'string' || !/^[a-f0-9]{64}$/i.test(body.txHash || '') || body.network !== 'preprod' || typeof body.contractAddress !== 'string' || !body.contractAddress || typeof body.blockReference !== 'string' || !body.blockReference) return json({ error: 'Complete Compact Preprod transaction evidence is required.' }, 400);
  ...
  const updated = { ...row, status: 'accepted', tx_hash: body.txHash.toLowerCase(), tx_network: 'preprod', tx_status: 'submitted', contract_address: body.contractAddress.slice(0, 200), block_reference: body.blockReference.slice(0, 200) }; const keys = await signingKeys(db); const receipt = receiptFromRow(updated); const signature = await signReceipt(receipt, keys); const now = new Date().toISOString();
  await db.batch([db.prepare(`UPDATE claim_records SET status = 'accepted', tx_hash = ?, tx_network = 'preprod', tx_status = 'submitted', contract_address = ?, block_reference = ?, signature = ?, updated_at = ? WHERE id = ? AND status = 'reserved'`).bind(updated.tx_hash, updated.contract_address, updated.block_reference, signature, now, body.id), db.prepare(`INSERT INTO claim_events (id, event_type, created_at) VALUES (?, 'compact_submitted', ?)`).bind(crypto.randomUUID(), now)]); return json({ receipt, signature, publicKey: keys.publicJwk });
}
